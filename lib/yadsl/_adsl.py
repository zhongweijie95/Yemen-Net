...
class Erros:
    @classmethod
    def err(cls, resp: requests.Response) -> bool:
...
class YADSL:
    ...
    def __init__(self, lang: str = "en", cookies: dict = None):
        self._lang = lang
    ...
    def login(self, username: str = None, password: str = None) -> int:
        ...
        _login_get_soup = BeautifulSoup(_login_get.content, "html.parser")
        ...
        _login_post_soup = BeautifulSoup(_login_post.content, "html.parser")
    ...
    def verify(self, captcha: str) -> tuple[str | None, requests.Response]:
        self._payload.set_captcha(captcha)
        resp = self._session.post(self._login_url, data=self._payload.data)
        return (resp, Erros.err(resp))

    def fetch_data(self, cookies: dict = None, formating: bool = True) -> dict:
        if cookies is not None:
            self.clear_cookies()
            self.set_cookies(cookies)
        ...
        _request_soup = BeautifulSoup(_request.content, "html.parser")
        ...
        _data.update(
            {
                k.text.strip().lower().replace(" ", "_" if formating else " "): v.text.strip().lower() 
